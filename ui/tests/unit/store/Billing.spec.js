...
describe('Billing', () => {
  ...
  it('Verify initial state change for setSubscription mutation', () => {
    ...
    console.log(store.getters['billing/get']);

    expect(store.getters['billing/get']).toEqual({...data, active: active, customer_id: customerId, subscription_id: subsId});
    expect(store.getters['billing/active']).toEqual(active);
  });
  ...
  it('Verify initial state change for setPaymentMethod mutation', () => {
    ...
    expect(store.getters['billing/get']).toEqual({...data, active: active, customer_id: customerId, subscription_id: subsId, payment_method_id: paymentMethodId});
    expect(store.getters['billing/active']).toEqual(active);
  });
